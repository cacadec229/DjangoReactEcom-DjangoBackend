...
def generate_session_token(length=10):
    return ''.join(random.SystemRandom().choice([chr(i)
    for i in range(97, 123)] + [str(i) for i in range(10)]) for _ in range(length))
    ...
    try:
        ...
        if user.check_password(password):
            ...
            if user.session_token != "0":
                ...
                user.save()
                return JsonResponse({'error': "Previous Sessions Exists"})
